...
class PassiveAggressiveRegressorModel(BaseRegressor, PassiveAggressiveRegressor):
    ...
    max_iter : int
        The maximum number of passes over the training data. Default is 1000.
    tol : float
        The stopping criterion. Default is 1e-3.
    early_stopping : bool
        Whether to use early stopping to terminate training when validation score does not improve. Default is False.
    validation_fraction : float
        The proportion of the training data to use as validation set when early stopping is enabled. Default is 0.1.
    n_iter_no_change : int
        The maximum number of iterations with no improvement before early stopping. Default is 5.
    shuffle : bool
        Whether to shuffle the training data before each epoch. Default is True.
    verbose : int
        The verbosity level. Default is 0.
    loss : str
        The loss function to use. Can be "epsilon_insensitive" or "squared_epsilon_insensitive". Default is "epsilon_insensitive".
    epsilon : float
        The epsilon parameter for the epsilon-insensitive loss function. Default is 0.1.
    random_state : int, RandomState instance or None
        The seed of the pseudo-random number generator. Default is None.
    warm_start : bool
        Whether to reuse the solution of the previous call to fit as initialization. Default is False.
    average : bool
        Whether to compute the averaged model. Default is False.
    """
